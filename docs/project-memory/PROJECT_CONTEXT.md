...
- [2026-08-24 18:58:09 UTC+07:00][recorded_at] P-TRACK invariant: canonical `pqg/smoke` is the aggregate status. `pqg/smoke-full` proves a completed full source-validation run; `pqg/tracking-integrity` proves only bounded allowlisted tracking-equivalence to that full-validated anchor and never proves runtime tests ran on the tracking SHA. This invariant becomes accepted durable CI context only when the exact T1 completion candidate has both `pqg/tracking-integrity=success` and `pqg/smoke=success`.
