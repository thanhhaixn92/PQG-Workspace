...
$backendProof = Test-PqgProcessRecord -Record $state.backend -Role backend -ExpectedWorkingDirectory $BackendDir -ExpectedCommand $BackendCommandIdentity -ExpectedIdentityMarker $backendMarker -ExpectedExecutable $powerShellExe -ExpectedDbPath $backendDbPath -RequireDbPath
