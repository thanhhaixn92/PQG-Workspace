...
def test_product_branding_is_separate_from_hermes_runtime() -> None:
    ...
    left_navigation = (
        repository / "frontend" / "src" / "foundation" / "shell" / "LeftNavigation.tsx"
    ...
    assert 'aria-label={PRODUCT_NAME}' in left_navigation
    assert "ASSISTANT_LABEL" in branding
    assert "Hermes Local" not in index_html
    assert "Hermes Local" not in left_navigation
