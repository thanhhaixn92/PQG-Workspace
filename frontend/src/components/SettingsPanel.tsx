...
import {
  checkGyoProvider, createGyoModel, createGyoProvider, getModelConfig, retireGyoModel,
  retireGyoProvider, setGyoProviderCredential, updateGyoModel, discoverGyoProviderModels,
  importZenFreeModels, installZenFreePreset, updateGyoRoutingPolicy,
  type GyoCostClass, type GyoModelTier, type GyoProviderCatalog, type GyoProviderType, type ModelConfig,
} from '../api/marketplace';
import { LocalDataPanel } from './LocalDataPanel';
import { MarketplacePanel } from './MarketplacePanel';
import { RuntimeStatusPanel } from './RuntimeStatusPanel';

type Section = 'model' | 'marketplace' | 'data' | 'advanced';
...
export const SettingsPanel: React.FC = () => {
  ...
  useEffect(() => { void reload().catch(() => setMessage('Chưa tải được cấu hình GYO. Bạn có thể thử lại.')); }, [reload]);
  ...
  const perform = async (key: string, action: () => Promise<unknown>, success: string) => {
    ...
    catch { setMessage('Không thể lưu cấu hình. Không có thay đổi nào được xác nhận; hãy kiểm tra lại và thử lại.'); }
  ...
  const addProvider = async (event: React.FormEvent) => {
    ...
    await perform('add-provider', () => createGyoProvider({
      display_name: providerForm.display_name.trim(), provider_type: providerForm.provider_type,
  ...
  const addModel = async (event: React.FormEvent) => {
    ...
    await perform('add-model', () => createGyoModel({
      provider_profile_id: modelForm.provider_profile_id, display_name: modelForm.display_name.trim(),
      model_identifier: modelForm.model_identifier.trim(), tier: modelForm.tier,
      capabilities: modelForm.tier === 'vision' ? ['chat', 'vision'] : ['chat'], priority: 100,
  ...
  const loadCatalog = async (providerId: string) => {
    setBusy(`catalog-${providerId}`); setMessage(null);
    try {
  ...
  const importCatalogModels = async (providerId: string) => {
    ...
    setBusy(`import-${providerId}`); setMessage(null);
    try {
  ...
  return <div className="grouped-panel">
    <div className="grouped-panel-tabs" aria-label="Cài đặt ứng dụng">
      <button className={section === 'model' ? 'active' : ''} onClick={() => setSection('model')}>GYO &amp; Model</button>
      <button className={section === 'marketplace' ? 'active' : ''} onClick={() => setSection('marketplace')}>Marketplace</button>
      <button className={section === 'data' ? 'active' : ''} onClick={() => setSection('data')}>Dữ liệu và backup</button>
      <button className={section === 'advanced' ? 'active' : ''} onClick={() => setSection('advanced')}>Chẩn đoán nâng cao</button>
    </div>
    <div className="grouped-panel-body">
      {section === 'model' && <section className="model-settings-panel" aria-labelledby="gyo-model-title">
        <header><h2 id="gyo-model-title">Bộ não của Trợ lý GYO</h2><p>GYO chỉ dùng provider và model bạn bật. Tự động chọn theo độ dài, tệp đính kèm và loại tác vụ; mỗi lượt vẫn lưu model đã dùng.</p></header>
        ...
        <dl><div><dt>Provider sẵn sàng</dt><dd>{activeProviders.filter(item => item.health_status === 'ready').length}</dd></div><div><dt>Model đang bật</dt><dd>{(model?.models ?? []).filter(item => item.enabled && !item.retired_at).length}</dd></div><div><dt>Model mặc định</dt><dd>{model?.model || 'Chưa chọn'}</dd></div></dl>
        ...
        <section className="gyo-routing-policy" aria-label="Fallback model">
          <div><strong>Fallback khi lỗi tạm thời</strong><small>Khi bật, GYO chỉ thử tối đa 2 model dự phòng trước token đầu tiên. Có thể dùng mọi model đang bật: {model?.routing_policy?.enabled_model_counts.free ?? 0} miễn phí, {model?.routing_policy?.enabled_model_counts.unknown ?? 0} chưa rõ chi phí, {model?.routing_policy?.enabled_model_counts.may_charge ?? 0} có thể tính phí.</small></div>
          <label className="toggle-label"><input type="checkbox" checked={model?.routing_policy?.auto_fallback_enabled ?? false} disabled={!model || busy === 'routing-policy'} onChange={event => void perform('routing-policy', () => updateGyoRoutingPolicy(event.target.checked), event.target.checked ? 'Đã bật fallback tối đa hai model dự phòng.' : 'Đã tắt fallback tự động.')} /> Bật fallback</label>
        </section>
        ...
        <div className="gyo-profile-list" aria-label="Provider và model GYO">
          {(model?.providers ?? []).filter(provider => !provider.retired_at).map(provider => <article key={provider.id}>
            <header><div><strong>{provider.display_name}</strong><small>{provider.provider_type} · {provider.health_message || provider.health_status}</small></div><span className={`status-badge ${provider.health_status}`}>{provider.health_status === 'ready' ? 'Sẵn sàng' : provider.health_status === 'needs_credential' ? 'Cần khóa' : 'Cần kiểm tra'}</span></header>
            <div className="gyo-profile-actions"><input type="password" aria-label={`Khóa API cho ${provider.display_name}`} autoComplete="new-password" value={credentialInputs[provider.id] ?? ''} onChange={event => setCredentialInputs(current => ({ ...current, [provider.id]: event.target.value }))} placeholder={provider.credential_configured ? 'Thay khóa API' : 'Nhập khóa API'} /><button className="btn-secondary compact-button" disabled={!credentialInputs[provider.id] || busy === `credential-${provider.id}`} onClick={() => void perform(`credential-${provider.id}`, async () => { await setGyoProviderCredential(provider.id, credentialInputs[provider.id]); setCredentialInputs(current => ({ ...current, [provider.id]: '' })); }, 'Đã cập nhật khóa cục bộ.')}>Lưu khóa</button><button className="btn-secondary compact-button" disabled={busy === `health-${provider.id}`} onClick={() => void perform(`health-${provider.id}`, () => checkGyoProvider(provider.id), 'Đã kiểm tra cấu hình provider.')}>Kiểm tra</button>{(provider.base_url || '').replace(/\/$/, '') === 'https://opencode.ai/zen/v1' && <button className="btn-secondary compact-button" disabled={!provider.credential_configured || busy === `catalog-${provider.id}`} onClick={() => void loadCatalog(provider.id)}>{busy === `catalog-${provider.id}` ? 'Đang tải…' : 'Cập nhật danh mục miễn phí'}</button>}<button className="btn-secondary compact-button" disabled={busy === `retire-provider-${provider.id}`} onClick={() => { if (window.confirm(`Ngừng dùng provider “${provider.display_name}”? Lịch sử cũ vẫn được giữ.`)) void perform(`retire-provider-${provider.id}`, () => retireGyoProvider(provider.id), 'Provider đã được ngừng dùng.'); }}>Ngừng dùng</button></div>
            ...
            {catalogs[provider.id] && <section className="gyo-catalog-list" aria-label={`Model OpenCode Zen cho ${provider.display_name}`}><div><strong>Thêm lựa chọn miễn phí</strong><small>Danh sách này chỉ dùng catalog live để kiểm tra model Free đã khóa vẫn còn khả dụng. Model chưa thêm sẽ không xuất hiện trong GYO.</small></div>{catalogs[provider.id].models.map(item => { const imported = (model?.models ?? []).some(profile => profile.provider_profile_id === provider.id && profile.model_identifier === item.model_identifier && !profile.retired_at); return <label key={item.model_identifier}><input type="checkbox" checked={imported || (catalogSelections[provider.id] ?? []).includes(item.model_identifier)} disabled={imported || busy === `import-${provider.id}`} onChange={() => toggleCatalogModel(provider.id, item.model_identifier)} /><span><strong>{item.display_name}</strong><small>{item.model_identifier} · {item.tier} · miễn phí{imported ? ' · đã thêm' : ''}</small></span></label>; })}{catalogs[provider.id].models.length === 0 && <p className="muted-copy">Hiện không có model Zen Free trong danh sách khả dụng. PQG Workspace không tự thêm model không xác định.</p>}<button className="btn-primary compact-button" disabled={busy === `import-${provider.id}` || !(catalogSelections[provider.id] ?? []).length} onClick={() => void importCatalogModels(provider.id)}>{busy === `import-${provider.id}` ? 'Đang thêm…' : 'Thêm lựa chọn miễn phí'}</button></section>}
            <div className="gyo-model-list">{(model?.models ?? []).filter(item => item.provider_profile_id === provider.id && !item.retired_at).map(item => <div key={item.id}><span><strong>{item.display_name}</strong><small>{item.model_identifier} · {item.tier} · {costLabel[item.cost_class]}{item.is_default ? ' · mặc định' : ''}</small></span><span className="gyo-model-actions"><button className="btn-secondary compact-button" onClick={() => void perform(`toggle-${item.id}`, () => updateGyoModel(item.id, { enabled: !item.enabled }), item.enabled ? 'Đã tắt model.' : 'Đã bật model.')}>{item.enabled ? 'Tắt' : 'Bật'}</button><button className="btn-secondary compact-button" disabled={item.is_default || !item.enabled} onClick={() => void perform(`default-${item.id}`, () => updateGyoModel(item.id, { make_default: true }), 'Đã đặt model mặc định.')}>Đặt mặc định</button><button className="btn-secondary compact-button" onClick={() => { if (window.confirm(`Ngừng dùng model “${item.display_name}”?`)) void perform(`retire-model-${item.id}`, () => retireGyoModel(item.id), 'Model đã được ngừng dùng.'); }}>Ngừng dùng</button></span></div>)}</div>
          </article>)}
      ...
      {section === 'data' && <LocalDataPanel />}
      {section === 'advanced' && <><div className="runtime-guidance">Khu vực này dành cho chẩn đoán runtime và kiểm tra kỹ thuật. Trạng thái “chưa xác định” không đồng nghĩa với “sẵn sàng”.</div><RuntimeStatusPanel /></>}
    </div>
  </div>;
};
