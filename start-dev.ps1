...
function Get-BackendCommandIdentity {
    param([int]$Port, [string]$DbPath, [bool]$Reload)
    $db = Get-PqgCanonicalPath $DbPath
    $material = "backend`n$Root`n$SourceSha`n$BackendDir`n$Port`n$db`n$Reload`nuvicorn app.main:app"
    return "uvicorn app.main:app | pqg-bind=$(Get-BindingDigest -Material $material)"
}

function Get-FrontendCommandIdentity {
    param([int]$Port, [int]$BackendPortValue)
    $material = "frontend`n$Root`n$SourceSha`n$FrontendDir`n$Port`n$BackendPortValue`nnpm run dev"
    return "npm run dev | pqg-bind=$(Get-BindingDigest -Material $material)"
}
...
if ($null -ne $state) {
    ...
    $backendCommandIdentity = Get-BackendCommandIdentity -Port $recordedBackendPort -DbPath $BackendDbPath -Reload ([bool]$state.backend.reload)
    $frontendCommandIdentity = Get-FrontendCommandIdentity -Port $recordedFrontendPort -BackendPortValue $recordedBackendPort
    ...
    if ($NoReload -and [bool]$state.backend.reload) { throw 'Invocation -NoReload khong duoc reuse backend recorded voi reload.' }
...
try {
    if ($null -eq $BackendRecord) {
        $reload = -not $NoReload
        $backendCommandIdentity = Get-BackendCommandIdentity -Port $BackendPort -DbPath $BackendDbPath -Reload $reload
    ...
    if ($null -eq $FrontendRecord) {
        $frontendCommandIdentity = Get-FrontendCommandIdentity -Port $FrontendPort -BackendPortValue $BackendPort
